...
public interface IRequestRepository extends JpaRepository<Request, Long> {
	...
	List<Request> findMatchingRequests(@Param(value = "assistanceSet") Set<Assistance> assistanceSet);

	@Query(value = "FROM Request WHERE status = :requestStatus AND client.id = :clientId")
	Set<Request> searchForOffers(@Param(value = "requestStatus") String requestStatus,
